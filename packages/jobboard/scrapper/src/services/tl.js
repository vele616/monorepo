const getUrls = async (browser, url) => {
  ...
  return await page.evaluate(() => ({
    ...
    companyName: document.querySelector('head > meta[property="og:site_name"]').content,
...
const getJobs = async (browser, url) => {
  ...
  return {
    ...await page.evaluate(() => {
      ...
      const parent = document.querySelector('div > div > div.col-md-8.col-sm-12.col-xs-12 > div.description.textColor');
      parent.removeChild(document.querySelector('div > div > div.col-md-8.col-sm-12.col-xs-12 > div.description.textColor > p:last-child'));

      return {
        title: document.querySelector('h1').textContent,
        content: parent.innerHTML.replace(/<br>/g, '')
        .replace(/h4/g, 'div')
        .replace(/h5/g, 'div')
        .replace(/h3/g, 'h2')
