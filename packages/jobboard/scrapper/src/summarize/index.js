...
const getPreposition = (word) => ['a', 'e', 'i', 'o', 'u'].includes(word[0]) ? 'an': 'a'
...
const createTLDR = (company, position, phrase, selectedRegex) => [
  ...
  {
    regex: [0, 1, 2, 3, 4],
    expression: `${company} is searching for ${getPreposition(position)} ${position} that has ${phrase}.`,
  },
  {
    regex: [5],
    expression: `${company} is looking for ${getPreposition(position)} ${position} that has ${phrase}.`,
  ...
  {
    regex: [0, 1, 2, 3, 4],
    expression: `If you have ${phrase}, ${company} has a job opening for ${getPreposition(position)} ${position}`,
  },
  {
    regex: [5],
  ...
  {
    regex: [0, 1, 2, 3, 4],
    expression: `Interested in new opportunities and have ${phrase}? ${company} has a job opening for ${getPreposition(position)} ${position}.`,
  },
  {
    regex: [0, 1, 2, 3, 4],
    expression: `To apply as ${getPreposition(position)} ${position} at ${company}, you preferably need to have ${phrase}.`,
  },
  {
    regex: [5],
